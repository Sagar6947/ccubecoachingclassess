...
include('config.php');
$cid_v = $_GET['id'];
$bm1 = mysqli_query($con, "SELECT * FROM `pdf_category` WHERE `id`  = '" . $cid_v . "'  ");
$as1 = mysqli_fetch_array($bm1);
$q = mysqli_query($con, "SELECT * FROM `video` WHERE `course_id` = '" . $cid_v . "' ORDER BY `unit` ASC ");
$rf = mysqli_fetch_array($q);
$bm = mysqli_query($con, "SELECT * FROM `main_courses` WHERE `id`  = '" . $as1['class_id'] . "'  ");
      ...
      <div class="row">
        ...
        <div class="tab-content " id="pills-tabContent" style="width:100%">
          <div class="row">
            <?php
            $i = 0;
            $rc = "SELECT * FROM `video` WHERE `course_id` = '" . $cid_v . "' ORDER BY `unit` ASC ";
            // echo $rc;
            $q = mysqli_query($con, $rc);

            while ($r = mysqli_fetch_array($q)) {
              echo '
                             <div class="col-md-4" style="margin-bottom: 25px;">
                                <div class="card card-event">
                                  <iframe allowfullscreen="0" src="' . str_replace('https://youtu.be/', 'https://www.youtube.com/embed/', str_replace('https://www.youtube.com/watch?v=', 'https://www.youtube.com/embed/', $r['url'])) . '" alt="" height="200px"></iframe>
                                  ...
                                  <div class="card-footer tx-13">
                                    ...
                                    <a href="../admin/' . $r['pdf'] . '" class="btn btn-xs btn-secondary" target="_blank" download> Download PDF</a>
                                  </div>
                                </div>
                              </div>
                               ';
