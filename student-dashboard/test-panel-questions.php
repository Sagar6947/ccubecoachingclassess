...
$qus1 = mysqli_query($con, "SELECT * FROM `pdf_category` WHERE `id`= '" . $ro['subject_id'] . "'");
$ros1 = mysqli_fetch_array($qus1);

$qusd = mysqli_query($con, "SELECT * FROM `main_courses` WHERE `id`= '" . $ros1['class_id'] . "'");
...
<body class="pos-relative" data-spy="scroll" data-target="#navSection" data-offset="120">
    ...
    <div class="section-nav" id="translate-this" style="background-color: #ccf1f6">
        ...
        <hr>
        <h5 class="mg-b-2 tx-spacing--1">Test Series</h5><br>
        <p><b>Test Name </b><br><?php echo $rosd['name'] . '
        <br>' . $ro['examtitle']; ?><br>
    ...
    <div class="content content-components">
        <form action="view_answer.php" method="post">
            <div class="container">
                <h1 class="df-title">C cube | <?php echo $ro['examtitle']; ?></h1>

                <?php

                $qs = mysqli_query($con, "SELECT * FROM `test_questions` WHERE `test_series_id`='" . $id . "' AND `quest_type`='0'");
                while ($r = mysqli_fetch_array($qs)) {
                    ...
                    echo '><div class="row">
                        <div class="col-md-4"><h6>Question no. ' . $j . ' </h6></div><div class="col-md-4"></div><div class="col-md-4" style="text-align:right;"><span class="translate  btn-warning" style="padding:5px;" data-id="' . $r['id'] . '" >English / Hindi</span></div></div><br>
                ...
                ?>
                            <div class="custom-control custom-radio">
                                <input type="radio" data-id="<?= $r['id'] ?>" id="customRadio1<?= $ip ?><?= $r['id'] ?>" name="customRadio<?= $r['id'] ?>" class="custom-control-input" value="<?= $arr[0] ?> | <?= $arr[1] ?>">
                                <label class="custom-control-label" for="customRadio1<?= $ip ?><?= $r['id'] ?>"><span class="english<?= $r['id'] ?>"> <?= $arr[0] ?></span>
                                    <span class="hindi<?= $r['id'] ?>" style="display:none;"><?= $arr[1] ?></span></label>
                            </div>
                <?php
                            $ip++;
                        }
                    }
                    echo '<div data-label="" class="df-example" style="border:0px">
                        <div class="demo-btn-group" style="float:right">';
                    $pre = mysqli_query($con, "select * from test_questions where id = (select max(id) from test_questions where `id`<'" . $r['id'] . "' and test_series_id = '" . $r['test_series_id'] . "' AND `quest_type`='0')");
                    ...
                    $ne = mysqli_query($con, "select * from test_questions where id = (select min(id) from test_questions where `id`>'" . $r['id'] . "' and test_series_id = '" . $r['test_series_id'] . "' AND `quest_type`='0')");
